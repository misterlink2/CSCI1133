...
class Minesweeper:
	...
	def clearCell(self,row,col):
		if self.countBombs(row,col) > 0:
			...
			#if self.__grid[row][col].isCleared()==False:#may need to remove this, this just makes the endless recursion stop for now
			self.__grid[row][col].clear()
			self.__grid[row][col].draw()
			self.__grid[row][col].showCount(self.countBombs(row,col))
			...
			if 0<row<13 and 0<col<13:
				for i in neighbors:
					for j in neighbors:
							...
							#else:
						print(row+i,col+j)
						self.__grid[row+i][col+j].clear()
						self.__grid[row+i][col+j].draw()
						self.__grid[row+i][col+j].showCount(self.countBombs(row+i,col+j))
						if i==1 and  j==1:
							for q in range(3):
								for w in range(3):					
									self.clearCell(row-1+q,col-1+w)#
			if row == 13 and 0<col<13:
				for i in neighbors[:-1]:
					for j in neighbors:
						if i==0 and  j ==0:
