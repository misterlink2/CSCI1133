...
def create():
	...
	A[(199)-int(origin)][int(origin)] = True 			#designate the origin as "True" in the matrix
									...
									#I set the matrix to look like the turtle display

	while particle_count < particles:				#so the Y axis is inverted
		...
		if round((x1+y1)**(.5)) == R+1:				#i just used the pythagorean theroem to determine if the randomly 

									#selected x and y values are at location 'R+1'

			random_walk = 0					#if not, the program just picks two new variables.
			...
			while random_walk < 200 :
				...
				if neighbor == True:			#If they are occupied
					...
					if R <= 24:
						...
						t.pencolor(255,5*R,0)

					if 24 < R < 51 :

						t.pencolor(255,125+2*R,0)

					if 51 <= R < 99:

						t.pencolor(255,225,-50+R)

					t.dot(5)

					if abs(origin-x)>maxdistance:	#updates R if the distance from the new particle is greater 

						maxdistance = abs(origin-x)

						R=R+1			#than the previous farthest one
					...
					particle_count+=1		#the particle counter increments only if the particle sticks
